...
{
    double distance_gt;
    if (!getInput("distance_gt", distance_gt)) {
        ROS_ERROR("[CheckDistanceSinceScoop] Missing required input 'distance_gt'");
        return BT::NodeStatus::FAILURE;
    }
...
{
    double distance_gt;
    if (!getInput("distance_gt", distance_gt)) { return BT::NodeStatus::FAILURE; }
...
BT::NodeStatus SendNavigationGoal::tick()
{
    ...
    topic_manager.sendMoveBaseGoal(goal.value(), static_cast<NavigationStatus>(status_in));
    return BT::NodeStatus::SUCCESS;
}
