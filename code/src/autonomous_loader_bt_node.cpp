...
private:
    ...
    void startTaskCallback(const autonomous_loader_msgs::TaskCommand::ConstPtr& msg)
    {
        ...
        if (msg->task_id == 0)
        {
            ROS_WARN("Received task cancellation request (task_id=0) via topic. Setting cancel flag.");
            autonomous_loader_bt::GlobalState::getInstance().setCancelTask(true);
            return;
        }
    ...
    bool taskServiceCallback(shuju::cangdou::Request& req, shuju::cangdou::Response& res)
    {
        ...
        if (req.taskID == 0) {
            ROS_WARN("Received task cancellation request (taskID=0). Setting cancel flag.");
            autonomous_loader_bt::GlobalState::getInstance().setCancelTask(true);
            res.huiying = true;
            return true;
        }
