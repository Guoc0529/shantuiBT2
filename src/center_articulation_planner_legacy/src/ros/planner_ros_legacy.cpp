...
// - clipPathByLookahead: 从路径最近投影点起向前裁剪固定弧长（不插入车位姿、不做 Hermite 衔接）。
...
namespace jhzx::center_articulation_planner
{
                                                 ...
                                                 double x_meter)
  {
    ...
    // 起点取路径上最近投影点，不再插入车位姿或 Hermite 衔接
    if (best_seg + 1 < path.poses.size())
    {
      const auto &p0 = path.poses[best_seg].pose.position;
      const auto &p1 = path.poses[best_seg + 1].pose.position;
      ...
      if (seg_len > 1e-9)
      {
        start_ps.pose.position.x = p0.x + best_t * seg_dx;
        start_ps.pose.position.y = p0.y + best_t * seg_dy;
        start_ps.pose.position.z = p0.z + best_t * (p1.z - p0.z);
        tf2::Quaternion q;
        q.setRPY(0, 0, std::atan2(seg_dy, seg_dx));
        start_ps.pose.orientation = tf2::toMsg(q);
      }
      else
      {
        start_ps = path.poses[best_seg];
        start_ps.header = out.header;
      }
      out.poses.push_back(start_ps);
    }

    extractPathSegment(path, best_seg, best_t, x_meter, out);
