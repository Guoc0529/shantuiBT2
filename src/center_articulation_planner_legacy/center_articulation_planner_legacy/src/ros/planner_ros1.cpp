...
namespace jhzx::center_articulation_planner
{
  ...
  void PlannerROS::onTimer()
  {
    ...
    case PlannerState::AdjustingDirection:
      ...
      if (need_inter_segment_drive_mode_switch_ && ros_io_)
      {
  ...
  void PlannerROS::planning_global_path()
  {
        ...
        !nav_path_.empty())
    {
      ...
      // original_goal.pose.position.x -= 0.7;
  ...
  void PlannerROS::applyGoalBodyOffset(geometry_msgs::PoseStamped &pose,
                                         double forward_m,
                                         double lateral_m)
  {
  ...
  {
    const double orig_x = plan_goal.pose.position.x;
    const double orig_y = plan_goal.pose.position.y;
    const double yaw = tf2::getYaw(plan_goal.pose.orientation);
    applyGoalBodyOffset(plan_goal, -unload_goal_offset_x_m_, 0.0); // 沿 goal 反向后退
    plan_goal.header.stamp = ros::Time::now();
    ROS_INFO("Unload goal offset (backward): yaw=%.2f, dist=%.2f, (%.2f,%.2f)->(%.2f,%.2f)",
             yaw,
             unload_goal_offset_x_m_,
             orig_x,
             orig_y,
             plan_goal.pose.position.x,
             plan_goal.pose.position.y);
  }
